...
const BVNForm = () => {
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      // BVN confirmed server-side and now sits PENDING_SELFIE_CONFIRMATION —
      // pass the verified bvn forward so the selfie step can complete the
      // combined verification in one call.
      navigation.navigate(
        "SelfieVerification" as never,
        { bvn: data?.bvn } as never,
      );
    } catch (err: any) {
      console.log(err);
      if (err instanceof AxiosError) {
