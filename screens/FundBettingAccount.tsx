...
export default function FundBettingAccountScreen() {
  ...
  // Refetch providers, beneficiaries, and both balances on screen focus
  useFocusEffect(
    useCallback(() => {
      refetchProviders();
      refetchBeneficiaries();
      refetchBalance?.();
      refetchPoints?.();
    }, [refetchProviders, refetchBeneficiaries, refetchBalance, refetchPoints]),
  );
  ...
  const onSubmit = useCallback(
    ...
    [providerOptions, saveBeneficiary, usePoints, navigation],
  );
  ...
  // Follows the blended funding model, same as the other bill-purchase
  // screens: usePoints=true checks points+wallet combined, not either alone.
  const hasInsufficientBalance = useMemo(() => {
