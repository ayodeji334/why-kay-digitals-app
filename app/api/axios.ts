...
const badRequestStatusCodes = [403, 422, 400, 500, 404, 401];
        ...
        if (error?.response?.status === 401 && !originalRequest._retry) {
          ...
          if (refreshToken) {
            try {
              ...
              logout();
              console.group("log user out! now from axios config");
            ...
            console.group(
              "logout user! now from axios config. becuase there is no refresh token",
            );
            logout();
            isRefreshing = false;
          }
